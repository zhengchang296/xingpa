该替换方式可跳过一帧一帧地更换图片及其图片对齐
目前仅限于修改已有的动画文件，因为我不会编写动画的代码，例如：walk_001、00--nn的json代码
...
接着将每个sprite的json导入。如果你导入的动画的帧数小于原动画，那可以直接删除多余的sprite文件，记得删数字最大的。如果导入帧数大于原动画，则需要修改你相应json文件中的m_name。
